...
src/
├── components/
│   ├── DataCleanup.tsx        # テストデータクリーンアップUI
│   ├── ui/
│   │   └── tabs.tsx           # Radix UIベースのタブコンポーネント
├── lib/
│   ├── utils.ts               # 共通ユーティリティ関数
│   └── cleanupTestData.ts     # テストデータクリーンアップ機能
```
...
src/
├── components/
│   ├── AdminPanel.tsx         # タブナビゲーション改善、削除機能追加
│   ├── AdvancedSearchFilter.tsx # 削除機能追加、テキスト表示改善
│   └── DataMigration.tsx      # タブナビゲーション改善、クリーンアップタブ追加
├── lib/
│   ├── supabase.ts            # 管理者用サービス追加、データ削除機能
│   └── deviceAuth.ts          # 非ラテン文字対応の修正
├── pages/
│   └── DiarySearchPage.tsx    # テキスト表示改善、ハイライト機能修正
```
